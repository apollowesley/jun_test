...
public class ReturnArrayMapMethodInfo extends AbstractWriteArrayMethodInfo
{
    ...
    public ReturnArrayMapMethodInfo(Method method, WriteStrategy strategy)
    {
    ...
    protected void writeOneDim(Class<?> rootType, String bk)
    {
        ...
        if (strategy != null)
        {
            str += bk + "\t\t\t\twriteStrategy.getWriter(entry.getKey().getClass()).write(entry.getKey(),cache);\n";
        }
        else
        {
        ...
        if (strategy != null)
        {
            str += bk + "\t\t\t\twriteStrategy.getWriter(entry.getValue().getClass()).write(entry.getValue(),cache);\n";
        }
        else
        {
