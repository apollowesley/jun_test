...
import com.jfire.codejson.function.ReaderContext;
                ...
                case '"':
                    ...
                    else
                    {
                        if (isJsonObject)
                        {
                            if (jsonKey == null)
                            {
                                jsonKey = new String(array, flag, index - flag);
                            }
                            else
                            {
                                ((JsonObject) jsonStack.peek()).put(jsonKey, new String(array, flag, index - flag));
                                jsonKey = null;
                            }
                        }
                        else
                        {
                            ((JsonArray) jsonStack.peek()).add(new String(array, flag, index - flag));
                        }
                        flag = 0;
                        strStartRead = false;
                    }
