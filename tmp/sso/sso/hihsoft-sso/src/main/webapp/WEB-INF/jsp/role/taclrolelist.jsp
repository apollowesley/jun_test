...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#btn_add").click(
				function() {
					win.window('open');
					$("#msg").load("${ctx}/taclRoleController.do?method=add",
							"", handler);
				});
		...
		$("#btn_edit")
				.click(
						function() {
							...
							var text = form.find(":checked").parent().parent()
									.find("td:eq(5)").text();
							...
							if (ids) {
								...
								$("#msg")
										.load(
												"${ctx}/taclRoleController.do?method=modifyInit",
												{
													id : ids
												}, handler);
							}
				...
				.click(
						function() {
							...
							if (!confirm("确定删除选中的记录吗？"))
								return;
							form.attr("action",
									"${ctx}/taclRoleController.do?method=delete&ids="
											+ getSelected());
							submit(
									form,
									function(res, status) {
										if (isSuccess(res, status)) {
											alert(
													...
													function() {
														form
																.attr("action",
																		"${ctx}/taclRoleController.do?method=list");
														form.submit();
													});
										} else {
											alert(
		...
		$("#btn_cancel").click(
				function() {
					var isGroup = $("#isGroup").val();
					var text = form.find(":checked").parent().parent().find(
							"td:eq(5)").text();
					if (isGroup != "true" && text == "公共") {
						alert("非总部用户不能修改取消分配公共角色");
						return;
					}
					ids = getSelected(true);
					if (ids) {
						win.window('open');
						$("#msg").load(
								"${ctx}/taclRoleController.do?method=userView",
								{
									id : ids
								}, handler);
					}
	...
	function showRole(id) {
		if (!id)
			return;
		$("#msg").load("${ctx}/taclRoleController.do?method=roleView&id=" + id,
				function() {
					win.window("open");
				});
	}
...
<body>
	<form method="post" target="_self"
		action="${ctx}/taclRoleController.do?method=list" name="listForm"
		...
		<div class="easyui-layout" style="border: none;" fit="true"
			...
			<div class="datagrid-toolbar" region="north" border="false"
				...
				<hih:auth operate="AUTHORIZATION" module="ACL_ROLE"
					value="button.assign.user" id="btn_authorization" iconCls="icon-ok"
					style="width: 70px;" />
				<hih:auth operate="CANCEL" module="ACL_ROLE"
					value="button.cancel.assign" id="btn_cancel" iconCls="icon-remove"
					style="width: 70px;" />
				...
				<table class="FormView" border="0" cellspacing="1" cellpadding="0"
					...
					<col class="Data" />
					<tr height="30px">
						<td align="left"><fmt:message key="taclrole.rolename" /></td>
						<td><input type="text" style="width: 200px"
							name="srh_rolename" value="${rolename}"></td>
						<td>&nbsp;&nbsp;</td>
						<td>&nbsp;&nbsp;</td>
					</tr>
				</table>
				...
				<table class="data-grid" cellspacing="0" cellpadding="0">
					...
					<c:forEach items="${list}" var="taclrole" varStatus="paStatus">
						<tr>
							...
							<td width="120"><a href="#"
								onclick="showRole('${taclrole.roleid}')">${taclrole.rolename}</a></td>
