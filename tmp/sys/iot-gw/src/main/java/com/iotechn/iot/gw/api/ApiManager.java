 ...
 * Time: 下午10:52
 */
public class ApiManager implements InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        //TODO 硬编码改配置
        this.registerService(Class.forName("com.iotechn.iot.executor.api.ExecutionOpenService"));
        this.registerService(Class.forName("com.iotechn.iot.user.api.UserOpenService"));
        this.registerService(Class.forName("com.iotechn.iot.device.api.DeviceOpenService"));
    }
        ...
        if (httpOpenApiAnnotation != null) {
            ...
            for (Method method : methods) {
                ...
                if(httpMethod != null){
                    ...
                    if (methodQuery != null) {
                        throw new GatewayServiceException(GatewayExceptionDefinition.GATEWAY_REGISTER_OPEN_API_FAILED);
                    }
                    tempMap.put(key, method);
                    logger.info("[注册OpenApi] " + group + "." + method.getName());
                }else{
                    logger.info("[注册OpenApi 失败] 没有注解");
                }
        ...
        for(String gpKey : gpKeys){
            ...
            for(String methodNameKey : methodNameKeys){
                ...
                if(httpMethod != null){
                    ...
                    if(retType.startsWith("com.iotechn.iot")){
                        //若返回值类型为复杂类型
