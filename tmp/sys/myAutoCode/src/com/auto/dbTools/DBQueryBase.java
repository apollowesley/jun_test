...
////					System.out.println("列头："+resMeta.getColumnName(i+1)+",列数据："+res.getObject(resMeta.getColumnName(i+1)));
...
//			}
			...
			boolean c = pstmt.execute();  //这点特别要注意:如果是Oracle而对于mysql可以不用加.
//			ResultSetMetaData rsmd = (ResultSetMetaData) pstmt.getMetaData();
			...
			e.printStackTrace();
			System.out.println(" com.auto.dbTools.DBQueryBase查询执行失败!");
		}finally{
