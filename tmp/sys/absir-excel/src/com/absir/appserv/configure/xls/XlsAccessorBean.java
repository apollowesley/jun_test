...
public class XlsAccessorBean extends XlsAccessor {
	...
	protected List<XlsAccessor> getXlsAccessors(final Class<?> beanClass, final XlsBase xlsBase) {
		...
		for (Field field : HelperAccessor.getFields(beanClass)) {
			...
			if (xlsBase.is(type)) {
				...
				if (type.isArray()) {
					...
					xlsAccessors.add(new XlsAccessorMap(field, KernelClass.rawClass(types[0]), KernelClass.rawClass(types[1]), xlsBase));
			...
			for (XlsAccessor accessor : accessors) {
				if (!accessor.isMulti()) {
					...
					for (int i = firstRow + 1; i < lastRow; i++) {
						String next = XlsAccessorUtils.getCellValue(hssfSheet.getRow(i).getCell(iColumn));
