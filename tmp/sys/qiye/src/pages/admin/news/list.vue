            ...
            <div style="display: flex;flex-direction: column;padding-top: 10px;">
                <div style="display: flex;justify-content: space-between;padding-bottom: 15px;padding-top:15px ;">
                	<div class="" style="align-self: flex-start;display: flex;">
                        <el-cascader v-model="searchCategoryCascader" expand-trigger="hover" filterable :options="categoryList" :props="props"   @change="cascaderChange">
                        </el-cascader>
                ...
                <template>
                    <el-table :data="userData" stripe type="index" border class="table" ref="multipleTable" max-height="700"
                        ...
                        <el-table-column prop="image" label="主图" min-width="80">
                    ...
                    <el-form    class="demo-table-expand" label-width="100px"  style="background: #fff;width: 90%;margin: 5%;height: 90%;" >
                        ...
                        <el-form-item label="图集" >
                            ...
                            <div class="block" style="width: 100%;height: 400px;">
                              ...
                              <el-carousel height="300px" width="400px" type="card">
                                <el-carousel-item v-for="item in imgList" :key="item.id">
                                  <img :src="getFileUrl(item.src,'image')" mode="cover" style="height: 300px;width: 400px; padding: 5px;" />
                                  <div>{{ item }}</div>
                                </el-carousel-item>
                              </el-carousel>
                            </div>
            ...
            <div class="form-box" style="align-self: center;">
            	<el-form ref="form" :model="form" label-width="100px" style="padding-top: 20px;">
            		...
            		<el-form-item  label="添加图片" prop="username">
            			<update-file openSort="true"  :controlledAdd="imageAdd"  :controlledDelete="imageDelete" :imgList="imgList" :max="max"  :maxwh="maxwh" :quality="quality" :change="fileChange"></update-file>	
            		</el-form-item>
...
<script>
    ...
    export default {
        ...
        created() {
            Request('AdminNewsCategory_List').then(e=>{
                // console.log(e)
                if(e.data.code==200){
        ...
        watch:{
            editVisible(e){
               if(e){
                  ...
                  this.categoryList=this.categoryList.filter((e)=>{
                        if(e.id!=0){
                            return e;
                        }
                    });
        ...
        methods: {
            ...
            imageAdd(e){
                if(e.src){
                 var data={t_id:this.detail.id};
                 data.image=e.src;
                 Request('AdminNewsImage_Upload',{data:data}).then(e=>{
                     var data=e.data;
                     if(data.code==200){
            ...
            imageDelete(i,e){
                ...
                if(e.src && e.id){
                 var data={id:e.id};
                 Request('AdminNewsImage_Delete',{data:data}).then(e=>{
                      console.log(e.data)
                     var data=e.data;
                     if(data.code==200){
            ...
            getDataDetail(id){
               if(id!=this.detail.id){
                    Request('AdminNews_Detail',{params:{id:id}}).then(e=>{
                        var data=e.data;
                        if(data.code==200){
                            ...
                            var imgList=[];
                            this.detail.news_image.filter(e=>{
                                imgList.push(e)
                            })
            ...
            onSubmit(formName) {
                ...
                var url = 'AdminNews_Add';
                switch (this.action) {
                    case 'add':
                        break;
                    case 'edit':
                    
                        url = 'AdminNews_Update';
                        var n=0;
                        var c=0;
                        var row=this.row;
                        for(let i in formUp){
            ...
            handleDelete(type, row, isTrue) {
                ...
                var caozuo = {
                    del: {
                        title: '删除',
                        api: 'AdminNews_Delete',
                    },
                    restore: {
                        title: '恢复',
                        api: 'AdminNews_Restore',
                    }
                }
                if (!caozuo[type]) {
