package club.lis.config;

import java.util.ArrayList;
import java.util.List;

import club.lis.model.Db;
import club.lis.model.Module;
import club.lis.model.PackageSetting;
import club.lis.model.TableConf;
import club.lis.util.XmlUtil;
...
public class Config {
	...
	public static Config loadConfig(){
		...
		pkgSetting.setObjectPackage(XmlUtil.getChild(pkg, "objectPackage").getTextTrim());
		...
		for (Element e : modules) {
			...
			elePkg = XmlUtil.getChild(e, "objectPackage");
			m.setObjectPackage(elePkg==null?pkgSetting.getObjectPackage():elePkg.getTextTrim());
	...
	private static TableConf initTableConf(Element e){
		...
		Attribute isAutoIncrement = XmlUtil.getAttribute(e, "isAutoIncrement");
		if (isAutoIncrement!=null) {
			m.setAutoIncrement(Boolean.valueOf(isAutoIncrement.getValue()));
		}
