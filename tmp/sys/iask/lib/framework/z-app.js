...
//				}
				// 上拉加载更多
				pullRefresh.up = {
					contentdown : init.pullRefresh.up.contentdown || "pull up can load more",
					contentrefresh : init.pullRefresh.up.contentrefresh || "loading...",
					contentnomore : init.pullRefresh.up.contentnomore || 'no more data',
					callback : function () {
...
//		});
	...
	this.openMenu = function(opts) {
		...
		var vid = url + "_" + styles.opener.id;
		var menuview = App.createWebview(url, vid, styles,{});
		
		var mask = opts.mask || 'rgba(0,0,0,0.5)';
		var maskview = opts.maskview;
		var anishow = opts.anishow || ' ';
		
		maskview.setStyle({mask: mask});
		
		menuview.show(anishow, 100);
		menuview.setStyle({showed:true, maskviewid : maskview.id});
		menuview.showed = true;
		// 点击遮罩层，隐藏菜单
		maskview.addEventListener("maskClick", function(e){
			_this.closeMenu({
				maskview : maskview,
				menuview : vid
			});
	...
	this.closeMenu = function(opts) {
		opts = opts || {};
		
		var menuview = plus.webview.getWebviewById(opts.menuview);
		var maskview = opts.maskview;
		
		menuview.showed = false;
		menuview.close('auto', 100);
		if (maskview) {
			// 移除掉遮罩层点击事件
	...
	this.toggleMenu = function(opts) {
		opts = opts || {};
		var url = opts.url;
		if (typeof url == 'undefined' || !url) {
			return;
		}
		var vid = url + "_" + plus.webview.currentWebview().id; 
		var menuview = plus.webview.getWebviewById(vid);
		var maskview = opts.maskview;
		if (menuview && ( menuview.showed == true || menuview.getStyle().showed == true ) ) {
			_this.closeMenu({
				menuview : vid,
				maskview : maskview
			});
