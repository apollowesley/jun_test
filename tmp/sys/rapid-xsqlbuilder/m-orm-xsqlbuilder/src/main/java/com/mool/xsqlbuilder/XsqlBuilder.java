...
import com.mool.datamodifier.DataModifierUtils;
...
public class XsqlBuilder
{
     ...
     * @param str 输入的字符串
     * @return 如果acceptedFilters存在{username}的key，则返回select * from user where username=str 否则没有影响，直接返回xsql
     */
     ...
     * @param acceptedFilters 过滤器
     * @param str 输入的字符串
     * @return 如果acceptedFilters存在{username}相对应的key=username的值，则返回select * from user where username=keyvalue 否则没有影响，直接返回xsql
     */
     ...
     * 使用过滤器过滤
     * 
     * @param xsql 数据格式：select * from user /where ~username={username}~/ 其中/~,~/为一个条件的结束与开始标志,{username}为过滤使用的key,key取值为username
     *            如果username在filters作为key存在,则返回结果为select * from user where username={username} 否则过滤后结果为select * from user
     * @param filters 过滤器,可以为Object or Map
     * @return
     */
