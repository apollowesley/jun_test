...
/**
 * 数据类型修饰工具类,可以将一种类型的数据修饰为另一种类型的数据 如String => Integer,String => Date的转换 例1：将String转为Integer, 表达式为var?int 例2：使用多个修饰，表达式为var?string?int 例3:
 * 为修饰传递参数,表达式为var?date(yyyy-MM-dd)
 * 
 * @author badqiu
 */
public class DataModifierBean
{
    ...
    private Map<String, DataModifier> modifiers = new HashMap<>();
    ...
    public Map<String, DataModifier> getAvailableDataModifiers()
    {
    ...
    private Object directModify0(String dataModifierExpression, Object dataModifyValue)
        throws Exception
    {
        ...
        while (tokenizer.hasMoreElements())
        {
            ...
            DataModifier dataModifier = modifiers.get(modifierName);
