...
public class XsqlBuilderTest extends TestCase
{
    ...
    public void testApplyFilters()
    {
        ...
        XsqlFilterResult result = builder.applyFilters(sql.toString(), filters);
        ...
        filters.put("age", "age");
        result = builder.applyFilters(sql.toString(), filters);
    ...
    public void testPerformence()
    {
        ...
        sql.append("/~ and username = {username}~/");
        sql.append("/~ and pwd = '[password]'~/");
        sql.append("/~ and pwd = '[23423password]'~/");
        sql.append("/~ and pwd = '[error_password]'~/");
        sql.append("/~ and age = {age}~/");
        ...
        {
            result = builder.generateSql(sql.toString(), filters, info).getXsql();
        }
    ...
    public void testDataModifier()
    {
        ...
        sql.append("/~age={age?long}~/");
        sql.append("/~birthDate={birthDate?timestamp}~/");
        sql.append("/~num=[num?string]~/");
        filters.put("age", "20");
    ...
    {
        // 清晰的sql语句,/~ ~/为一个语法块
        String sql = "select * from user where 1=1 " + "/~ and username = {username} ~/" + "/~ and password = {password} ~/";
    ...
    {
        XsqlFilterResult result;
        try
        {
            result = new XsqlBuilder().generateHql(null, new HashMap<>());
            fail();
        }
        catch (IllegalArgumentException e)
        {
        }
        result = new XsqlBuilder().generateHql("", null);
    ...
    public void testBean()
    {
        ...
        BlogInfo info = new BlogInfo();
        info.setTitle("java");
        XsqlFilterResult result = new XsqlBuilder().generateHql(sql.toString(), info);
    ...
    {
        // 清晰的sql语句,/~ ~/为一个语法块
        String sql = "select * from user where 1=1 " + "/~ and title = {title} ~/" + "/~ and sex = {sex} ~/" + "/~ and salary = {salary} ~/" + "/~ and age = [age] ~/" + "/~ and mapKey = [mapKey] ~/";
    ...
    {
        // 清晰的sql语句,/~ ~/为一个语法块
        String sql = "select * from user where 1=1 " + "/~ and title = {title} ~/" + "/~ and salary = {2009salary} ~/";
