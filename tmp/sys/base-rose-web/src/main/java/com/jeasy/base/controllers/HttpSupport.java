...
public abstract class HttpSupport {
    ...
    protected final int getPageNo() {
        int defaultPage = 1;
        int page = NumberUtils.toInt(inv.getRequest().getParameter("pageNo"), defaultPage);
        return page > 0 ? page : defaultPage;
    }

    protected final int getPageSize() {
        int defaultPageSize = 5;
        int pageSize = NumberUtils.toInt(inv.getRequest().getParameter("pageSize"), defaultPageSize);
