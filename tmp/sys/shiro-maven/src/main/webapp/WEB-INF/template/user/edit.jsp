...
<head>
    ...
    <link rel="stylesheet" href="${request.contextPath}/static/JQuery zTree v3.5.15/css/zTreeStyle/zTreeStyle.css">
    <style>
...
<body>
    ...
    <form method="post" commandName="user">
        ...
        <c:if test="${op eq '新增'}">
            <div class="form-group">
                ...
                <password path="password" name="password" value="${(user.password)!}"/>
            </div>
        </c:if>

        <div class="form-group">
            <label path="organizationId">所属组织：</label>
            <hidden path="organizationId"/>
        ...
        <div class="form-group">
            <label path="roleIds">角色列表：</label>
            <select path="roleIds" items="${roleList}" itemLabel="description" itemValue="id" multiple="true"/>
            (按住shift键多选)
        </div>
    ...
    <script src="${request.contextPath}/static/JQuery zTree v3.5.15/js/jquery.ztree.all-3.5.min.js"></script>
    <script>
        $(function () {
            var setting = {
                view: {
