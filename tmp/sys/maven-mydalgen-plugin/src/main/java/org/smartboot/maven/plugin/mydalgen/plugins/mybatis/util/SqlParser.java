 ...
 * 
 * @author Seer
 * @version SqlParser.java, v 0.1 2015年7月25日 上午10:24:24 Seer Exp.
 */
public class SqlParser {
	...
	public SqlParser(String sql) {
		...
		try {
			...
			LogUtils.error("Failed to parse sql statement " + sql + " due to exception.", pe);
		}
		...
		if (zst != null) {
			if (zst instanceof ZQuery) {
				...
				Vector<String> columns = ((ZInsert) zst).getColumns();
				Vector<ZExp> values = ((ZInsert) zst).getValues();
	...
	public String getTableName() {
		...
		if (zst instanceof ZQuery) {
			// a select statement
			Vector<ZFromItem> from = ((ZQuery) zst).getFrom();
			ZFromItem fromItem = from.get(0);
	...
	protected List<String> getParams(ZExp exp) {
			...
			if (opNum == 1) {
				...
				if (ol instanceof ZConstant && or instanceof ZExpression) {
					...
					if (cl.getType() == ZConstant.COLUMNNAME) {
						// TODO: an overly simplified approach, need more
						// processing
	...
	public boolean isSelectItemSingle() {
		if (zst instanceof ZQuery) {
			Vector<ZSelectItem> items = ((ZQuery) zst).getSelect();

			if (items.size() == 1) {
				ZSelectItem item = items.get(0);
	...
	public ZSelectItem getSelectItem() {
		if (isSelectItemSingle()) {
			return ((ZQuery) zst).getSelect().get(0);
		} else {
			return null;
		}
	}
