...
/**
 * @author Seer
 * @version IWalletConfig.java, v 0.1 2015年7月25日 上午10:27:30 Seer Exp.
 */
public class IWalletConfig {
	...
	public static synchronized void init(File configFile) throws IWalletConfigException {
		...
		try {
			...
			digester.addCallMethod("tables/typemap", "addJavaTypeMap", 2, new String[] { "java.lang.String",
			"java.lang.String" });
			...
			if (!includes.isEmpty()) {
				...
				digester.addCallMethod("table/mybatis", "addMybatis", 0);
				// 票据ID
				// digester.addSetProperties("table", "ticket", "ticket");
				// digester.addSetProperties("table", "ticketName",
				// "ticketName");
				...
				// parse sql
				// digester.addObjectCreate("table/sql",
				// IWalletSqlConfig.class);
				...
				// ------
				// digester.addSetProperties("table/operation", "paging",
				// "paging");
				digester.addSetProperties("table/operation", "resultMap", "resultMap");
				digester.addSetProperties("table/operation", "resultType", "resultType");
