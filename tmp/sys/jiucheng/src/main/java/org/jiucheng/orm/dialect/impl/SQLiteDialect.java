...
public class SQLiteDialect extends AbstractDialect {
    ...
    private static final Logger log = Logger.getLogger(SQLiteDialect.class);
    ...
    public <T> void forSave(T entity, Sql sh) {
        ...
        for (ColumnMapping cm : cms) {
            ...
            if (!tableMapping.isDynamic() || result != null) {
                before.append(cm.getColumnName()).append(",");
                after.append("?,");
    ...
    public <T> void forUpdateById(T entity, Sql sh) {
        ...
        sh.append("UPDATE ").append(tableName).append(" ");
        Object result = null;
        List<ColumnMapping> cms = tableMapping.getColumnMappings();
        int i = 0;
        try {
            for (ColumnMapping cm : cms) {
                if (!tableKey.equals(cm.getColumnName())) {
                    ...
                    if (!tableMapping.isDynamic() ||  null != result) {
                        sh.insertValue(result);
                        if (i != 0) {
                            sh.append(",").append(cm.getColumnName()).append("=?");
                        } else {
                            sh.append(" SET ").append(cm.getColumnName()).append("=?");
                        }
                        i++;
                    }
                }
    ...
    private static <T> void handleAfterWhere(T entity, Sql sh){
        ...
        try {
            ...
            for (ColumnMapping cm : tableMapping.getColumnMappings()) {
                ...
                if (null != result) {
                    if (hasWhere) {
                        sh.append(" AND ").append(tableColumn).append("=?");
                    } else {
                        sh.append(" WHERE ").append(tableColumn).append("=?");
                        hasWhere = true;
                    }
                    sh.insertValue(result);
                }
