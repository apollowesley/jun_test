...
<body>
	<div class="pageHeader" style="border: 1px #B8D0D6 solid">
		<form id="pagerForm" method="post"
			action="RoleAction_toRoleManagePage"
			...
			<table class="searchContent">
				<tr>
					<td>角色名：<input type="text" name="roleName" id="roleName"
	...
	<div class="pageContent"
		...
		<s:else>
			<div class="panelBar">
				<ul class="toolBar">
					<li><s:a cssClass="add" action="RoleAction_toAddRolePage"
						target="dialog" width="600" height="550"><span class="add">新增</span></s:a></li>
					<li><s:a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids"
						action="RoleAction_bacthDeleteRoles" postType="string"
				...
				<table class="list" width="100%" targetType="navTab" layoutH="90">
					<thead>
						<tr>
							...
							<th width="60" orderField="roleName"
								<c:if test='${param.orderField=="roleName" }'>class="${param.orderDirection}"</c:if>>角色名：</th>
					...
					<tbody>
						<s:iterator var="role" value="pageResult.list" status="sta">
							<tr target="roleId" rel="${role.id }">
								<td><input name="ids" type="checkbox" value='${role.id }'></td>
								<td>${role.roleName}</td>
								<td><div class="cut">${role.roleDesc}</div></td>
								<td><fmt:formatDate value="${role.regdate}"
										pattern="yyyy-MM-dd hh:mm:ss" /></td>
								<td class="operate">
									<s:a cssClass="buttons small orange" style="text-decoration: none" action="RoleAction_toAlterRolePage?id=%{id }" target="dialog" width="600" height="550"><span>修改角色</span></s:a>
								</td>
							</tr>
						</s:iterator>
					</tbody>
