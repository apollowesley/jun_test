...
<form id="pagerForm" method="post" action="QuestionAction_doTopic?questionPage=questionIndexPage"
	...
	<input type="hidden" name="questionsType"
		...
		value="${pageResult.page.numPerPage}" />
		<input type="hidden"
...
<body>
	...
	<div class="pageHeader" style="border: 1px #B8D0D6 solid">
		<form target="navTab" rel="pageForm" action="QuestionAction_doTopic?questionPage=questionIndexPage"
			...
			<table class="searchContent">
				<tr>
					...
					<td>试题模板： <input name="templateForm.id" value="${templateForm.id }" type="hidden" />
						<input id="" name="templateForm.name"
						type="text" value="${templateForm.name }" readonly> <a class="btnLook"
						href="latar/treeLookup.jsp" lookupGroup="templateForm"
						style="float: right;">查找带回</a>
					</td>
		...
		<s:else>
			<div>
				<div id="questions_list_print">
					<table class="list" width="90%" targetType="navTab" layoutH="91">
						<thead>
							<tr>
							...
							<s:iterator var="question" value="pageResult.list" status="sta">
								<tr target="questionId" rel="${question.id }">
									...
									<td><div class="cut"><s:a style="color: #000;"
										...
										target="dialog" title="试题显示">${question.title}</s:a></div></td>
									<td><s:if test="%{#type==2}">
