...
Page({
        ...
        topData:[
          ...
          },
          {
            content: {
              ...
              html: ' <p wx:nodeid="58">新增1</p>',
              text: "新增1"
            },
            date: '2019-12-06 22:54:05',
            bgColor: 'rgba(255, 2, 2,0.06)',
            isCheck: false
          }
        ],
        data:[
  ...
  onLoad: function (options) {
    ...
    let query=wx.createSelectorQuery();
        query.select('#swiperItem0').boundingClientRect();
        query.exec(res=>{
          this.setData({
            swiperHeight:res[0].height + 10 + 'px'
          })
        })
  },
  //点击切换
  tabClick: function (e) {
    ...
    if(currIndex === 0){
      ...
      isBtn = false;
      let query = wx.createSelectorQuery();
      query.select('#swiperItem1').boundingClientRect();
      query.exec(res => {
        this.setData({
          swiperHeight: res[0].height + 10 + 'px'
        })
...
//  下拉事件
  ...
  toOperate(){
    ...
    // 设置swiper高度
    let query = wx.createSelectorQuery();
    query.select("#operateGroup").boundingClientRect();
    query.exec(res => {
      console.log(res)
      this.data.swiperHeight = parseInt(this.data.swiperHeight) + res[0].height + 'px';
      this.setData({
  ...
  cancleOperate(){
    let query = wx.createSelectorQuery();
    if (query.select("#operateGroup").length>0){
      query.select("#operateGroup").boundingClientRect();
      query.exec(res => {
        this.data.swiperHeight = parseInt(this.data.swiperHeight) - res[0].height + 'px';
        this.setData({
  ...
  onReachBottom(){
    if(this.data.activeIndex ==0){
      ...
      let query = wx.createSelectorQuery();
      query = query.select('#swiperItem0').boundingClientRect();
      query.exec(res => {
        this.setData({
          swiperHeight: res[0].height + 'px'
        })
      });
      this.data.recently.topData.forEach(item=>{
        this.data.recently.topData.push(item);
      })
      this.data.recently.data.forEach(item=>{
        this.data.recently.data.push(item);
      });
      setTimeout(()=>{
        this.setData({
          recently:this.data.recently
