...
Page({
  data: {
    ...
    groupId:'',  //当前分组索引
    memoWrapperHeight: '', //memoWrapperHeight
    hasMore:false,
    loadText:'加载中...'
  },
//页面加载时执行 
...
// status 新建和便签展示在同一页 status区分新建或展示
  ...
  toOperate() {
    ...
    query0.exec(res => {
      heightMemo = res[0].height
    })
    let query1 = wx.createSelectorQuery();
    query1.select("#operateGroup").boundingClientRect();
    query1.exec(res => {
      this.data.memoWrapperHeight = res[0].height + heightMemo + 'px';
      this.setData({
        memoWrapperHeight: this.data.memoWrapperHeight
      })
    })   
  },
  // 取消操作
  ...
  onReachBottom(){
      ...
      // query = query.select('#memoWrapper').boundingClientRect();
      // query.exec(res => {
      //   this.setData({
      //     memoWrapperHeight: res[0].height + 'px'
      //   })
      // });
      this.data.topData.forEach(item => {
        this.data.topData.push(item);
      })
      this.data.normalData.forEach(item => {
        this.data.normalData.push(item);
      });
      setTimeout(() => {
        this.setData({
          topData: this.data.topData,
