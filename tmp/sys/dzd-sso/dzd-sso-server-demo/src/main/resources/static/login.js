var login = {
    ...
    login: function() {
        if (!login.isLogining) {
            var a = document,
            b = a.getElementById("account"),
            c = a.getElementById("password")
            verycode= a.getElementById("verycode");
            ...
            login.isLogining = !0,
            a.getElementById("account").value = b.value.trim(),
    ...
    paltform: {
        ...
        login: function(a) {
            if ("google" == a) {
                ...
                var c = "https://graph.qq.com/oauth2.0/authorize?client_id=101496501&response_type=code&scope=all&redirect_uri=http://www.jieerkangxidi.com/sns/callback/qq&state=admin";
                window.open(c, "newwindow", "height=550, width=900, top=0, left=0, toolbar=no, menubar=no, scrollbars=no,resizable=no,location=no, status=no")
            }  else if ("baidu" == a) {
                c = "https://openapi.baidu.com/oauth/2.0/authorize?response_type=code&client_id=ruoQHn4Avs6GRXarcsQ9Aa5i&redirect_uri=http://www.jieerkangxidi.com/sns/callback/baidu&display=page&state=admin";
                window.open(c, "newwindow", "height=550, width=900, top=0, left=0, toolbar=no, menubar=no, scrollbars=no,resizable=no,location=no, status=no")
            }
            else if ("csdn" == a) {
                c = "http://api.csdn.net/oauth2/authorize?client_id=1100872&redirect_uri=http://www.jieerkangxidi.com/sns/callback/csdn&response_type=code&state=admin";
                window.open(c, "newwindow", "height=550, width=900, top=0, left=0, toolbar=no, menubar=no, scrollbars=no,resizable=no,location=no, status=no")
            }else if ("oschina" == a) {
                c = "https://www.oschina.net/action/oauth2/authorize?response_type=code&client_id=vXAHX2MnjDsMjsWAERBh&redirect_uri=http://www.jieerkangxidi.com/sns/callback/oschina&state=admin";
                window.open(c, "newwindow", "height=550, width=900, top=0, left=0, toolbar=no, menubar=no, scrollbars=no,resizable=no,location=no, status=no")
            }
            else if ("sina" == a) {
                var c = "https://api.weibo.com/oauth2/authorize?client_id=1185373104&response_type=code&redirect_uri=http://www.jieerkangxidi.com/sns/callback/sina&state=admin";
                window.open(c, "newwindow", "height=550, width=900, top=0, left=0, toolbar=no, menubar=no, scrollbars=no,resizable=no,location=no, status=no")
            } else if ("mingdao" == a) {
                  ...
                  /*  var objWx = new WxLogin(
                    ...
                    setTimeout(function() {
                        new WxLogin({
                            ...
                            appid: "wx22db67a3c7637aef",
                            scope: "snsapi_login",
                            redirect_uri:encodeURIComponent("http://www.jieerkangxidi.com/sns/callback/wx"),
                            state: "admin",
                            style: "",
                            href: ""
                        });
