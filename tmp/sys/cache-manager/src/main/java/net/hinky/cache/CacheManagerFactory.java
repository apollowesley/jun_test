...
import net.hinky.cache.ehcache.EhCacheCacheManager;
import net.hinky.cache.memcached.MemcachedManager;
import net.hinky.cache.redis.RedisCacheManager;
...
public final class CacheManagerFactory {
	...
	private CacheManager getCacheManagerInstance(String managerName) {
		...
		// 获得缓存配置地址
		String configLocation = props
				.getProperty("cache.config." + managerName);
	...
	private InputStream getConfigFileStream(String configFile) {
		InputStream configStream = CacheManager.class.getClassLoader()
				.getParent().getResourceAsStream(configFile);
