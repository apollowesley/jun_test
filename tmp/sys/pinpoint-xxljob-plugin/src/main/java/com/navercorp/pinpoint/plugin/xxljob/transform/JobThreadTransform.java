...
public class JobThreadTransform implements TransformCallback {
    ...
    public byte[] doInTransform(Instrumentor instrumentor, ClassLoader classLoader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classfileBuffer) throws InstrumentException {
        ...
        if(constructor != null){
            ...
            constructor.addScopedInterceptor(AsyncConstructorInterceptor.class.getName(), scope, ExecutionPolicy.INTERNAL);
        }
        //获取声明的run方法，注入拦截器
        InstrumentMethod run = target.getDeclaredMethod("run");
        if(run != null){
            run.addInterceptor(AsyncRunInterceptor.class.getName());
        }
