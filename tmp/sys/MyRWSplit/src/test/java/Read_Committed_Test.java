...
public class Read_Committed_Test {
	private static LoggerHelper LOGGER = LoggerHelper.getLogger(Read_Committed_Test.class);
				...
				try {
					result = new MyRwSplitTransactionRunnable<Role>(ExecutorType.SIMPLE,
						...
						public Role execute(SqlSession sqlSession) {
							...
							// 再次查询
							LOGGER.info("第2个线程接着查询14");
							role = userMapper.getRole0(14);
							if (null != role)
								LOGGER.info("标题--->" + role.getTitle());
							else {
								LOGGER.info("第2个线程没有查到数据");
							}
							LOGGER.info("第2个线程全部执行完毕");
							return role;
						}
					}.run();
				} catch (Exception e) {
