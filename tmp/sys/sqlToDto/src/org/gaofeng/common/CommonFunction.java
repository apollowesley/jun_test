...
public class CommonFunction {
	 ...
	 */
	public static String createView(String stringSqlIn, int count) throws SQLException, ClassNotFoundException {
	 ...
	 * @throws ClassNotFoundException
	 */
	public static void dropView(String viewName) throws ClassNotFoundException, SQLException {
	...
	public static void makeDomain(String tableName) throws Exception {
		...
		if (MappingRule.ifMapper) {
			MakeMapper.makeMapper(groupTableViewModel, tableName);
		}
	...
	/**
	 * 根据数据库表名，或者视图名，通过数据库自身属性获得字段信息</br>
	 * 
	 * @param tableName
	 *            表名或者视图名
	 * @return
	 ...
	 * @throws Exception
	 */
	protected static List<TableViewModel> getColsByTableName(String tableName) throws ClassNotFoundException, SQLException, IOException, Exception {
	...
	private static GroupTableViewModel groupColByPriKey(List<TableViewModel> listCol) {
		...
		for (TableViewModel tb : listCol) {
			if (tb.getIsPrimaryKey()) {
				...
				if (list.getListUnKey() != null && list.getListUnKey().size() != 0) {
	...
	/**
	 * MySQL数据库通过表名或者视图名获得该表或视图的所有列信息</br> 包括，列名称、列类型、小数点、备注、是否为主键
	 * 
	 * @param tableName
	 * @return
	 * @throws ClassNotFoundException
	 ...
	 * @throws IOException
	 */
	public static List<TableViewModel> getColInfoMysql(String tableName) throws ClassNotFoundException, SQLException, IOException {
		sql = " SELECT COLUMN_NAME,DATA_TYPE,NUMERIC_SCALE, COLUMN_COMMENT,COLUMN_KEY ,COLUMN_TYPE  FROM information_schema.COLUMNS WHERE TABLE_NAME = '" + tableName + "'  and TABLE_SCHEMA='" + PropertiesTool.TABLE_SCHEMA + "'";
		db = new DBHelper(sql);
		...
		while (ret.next()) {
			...
			model.setColumnType(ret.getString(6));
	...
	private static void getCommentFromLoc(TableViewModel model, String tableName) {
		if (PropertiesTool.commentsMap.get(tableName.toUpperCase()) != null) {
			if (PropertiesTool.commentsMap.get(tableName.toUpperCase()).get(model.getFieldName().toUpperCase()) != null) {
				model.setComment(PropertiesTool.commentsMap.get(tableName.toUpperCase()).get(model.getFieldName().toUpperCase()));
			}
		}
	...
	/**
	 * Oracle数据库通过表名或者视图名获得该表或视图的所有列信息</br> 包括，列名称、列类型、小数点、备注、是否为主键,尚未完工
	 * 
	 * @param tableName
	 * @return
	 * @throws Exception
	 */
	public static List<TableViewModel> getColInfoOracle(String tableName) throws Exception {
		sql = "SELECT T.COLUMN_NAME,T.DATA_TYPE,T.DATA_SCALE ,k.comments,(select count(1) from user_cons_columns cu, user_constraints au " + " where cu.constraint_name = au.constraint_name and au.constraint_type = 'P' and au.table_name =t.TABLE_NAME and cu.column_name=t.column_name) as prikey   FROM USER_TAB_COLS T ,user_col_comments k " + " WHERE t.TABLE_NAME=k.table_name and t.COLUMN_NAME=k.column_name and  T.TABLE_NAME='" + tableName + "'";
		db = new DBHelper(sql);
		List<TableViewModel> listCol = new ArrayList<TableViewModel>();
		ret = db.pst.executeQuery();
		while (ret.next()) {
	...
	public static String getNumbers(TableViewModel tableViewModel) {
		...
		if ("tinyint".equals(tableViewModel.getType()) && "tinyint(1)".equals(tableViewModel.getColumnType())) {
	...
	public static void sysLog(String log, String lev) {
		...
		if (PropertiesTool.getInstance().LOGLEV.get(lev) != null && PropertiesTool.getInstance().LOGLEV.get(lev)) {
