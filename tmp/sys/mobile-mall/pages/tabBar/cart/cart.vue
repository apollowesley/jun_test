				...
				<view class="carrier" :class="[theIndex==index?'open':oldIndex==index?'close':'']" @touchstart="touchStart(index,$event)" @touchmove="touchMove(index,$event)" @touchend="touchEnd(index,$event)">
					...
					<view class="goods-info" @tap="toGoods(row)">
						...
						<view class="info">
							...
							<view class="spec">{{row.productAttr}}</view>
	...
	export default {
		...
		methods: {
			...
			joinGoods(goods){
				...
				* 在实际应用上，前端并不会直接插入记录到cartList这一个动作，一般是更新列表和本地列表缓存。
			...
			toConfirmation(){
				...
				uni.navigateTo({
					url: `/pages/order/confirmation?cartIds=${cartIds}&&type=2`
				});
				...
				for(let i=0;i<len;i++){
					if(this.cartList[i].selected) {
						if(e && i==index){
							...
							this.sumPrice = this.sumPrice + (this.cartList[i].quantity*this.cartList[i].price);
						}
					}
				}
