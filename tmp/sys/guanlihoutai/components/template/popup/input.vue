...
<script>
    //使用方法开启，传参 this.show({title:'提示',content{type:button|input|textarea|checkbox}});
    //或者 this.$refs.popupInput.show(true, (e => {
    // if (e.type == 'button') {
    ...
    export default {
        ...
        methods: {
            ...
            show(param, callback) {
                if(param){
                 this.callback = callback || function(e) {
                     console.log('请在show方法的第二个参数传入回调函数')
                 };
                 if (typeof param === 'object') {
                 
                     this.param = Object.assign({},this.contentDefault,param)
                 } else {
                 
                     this.param = this.contentDefault
                 }   
                }else{
                    this.param=null;
                }
            ...
            tapBtn(data) {
                ...
                data.type = data.type || data.items.type || data.item.type || '';
                if (this.param) {
                    if (this.param.success && typeof this.param.success == 'function') {
