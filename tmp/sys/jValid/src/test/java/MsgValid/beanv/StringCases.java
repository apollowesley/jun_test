...
//可选的不否和规则结果
		...
		assertEquals(false, MsgValidator.valid(cup0, cfgs));
...
//		1.length - 唯一长度3
		...
		assertEquals(true, MsgValidator.valid(cup1, cfgs));
...
//		1.length - 不能多于6个
		...
		assertEquals(false, MsgValidator.valid(cup1, cfgs));//11<=6
		assertEquals(true, MsgValidator.valid(cup2, cfgs));//6<=6
		assertEquals(true, MsgValidator.valid(cup3, cfgs));//5<=6
	}
...
//		assertEquals(true, VliCore.valid(cup1, cfgs));
		assertEquals(false, MsgValidator.valid(cup2, cfgs));//每个规则的校验结束都要判断，一旦false就立即返回验证结果
