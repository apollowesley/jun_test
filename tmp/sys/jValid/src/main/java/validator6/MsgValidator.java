...
public class MsgValidator {
	...
	private static ThreadLocal<String> msgTL = new ThreadLocal<String>();
	
	static{
		msgTL.set("校验通过");//设置默认消息
	}
	...
	public static boolean valid(JSONArray array, String config){
		return new MsgValidator().check(array, config);
	}
	/**
	 * 对象集合的校验
	 * @param list
	...
	public static boolean valid(List<Object> list, String config){
		return new MsgValidator().check(list, config);
	}
	/**
	 * 单一bean对象的校验
	 * @param obj
	...
	public static boolean valid(Object obj, String config){
		return new MsgValidator().check(obj, config);
	}
	/**
	 * 单字符串的校验
	 * @param obj
	 ...
	 * @return
	 */
	public static boolean valid(String obj, String config){
		return new MsgValidator().check(obj, config);
	}
	/**
	 * JSONArray验证
	 ...
	 * 字符串的验证
	 */
	public boolean check(String vvv, String config){
		...
		String option = cfgs.get(key).toString();//获取校验规则

		String val = null;
		
		if(vvv == null){
			if(option.startsWith("required")){
				msgTL.set("参数校验失败：["+key+":"+vvv+"]->规则[required]");
				return false;
			}else{
		...
		if(option.startsWith("required")){
			...
			if(option.contains(";")){//多则校验
				...
				for (int i = 0; i < rules.length; i++) {//规则循环校验
					result = ssingleValid(rules[i], val, result);
					if(!result)	return result;//一旦校验到false就返回
				}
			}else{
				result = ssingleValid(option, val, result);
				if(!result) return result;//一旦校验到false就返回
			}
		}
	 ...
	 * @return
	 */
	private boolean ssingleValid(String rule, String val, boolean result) {
		...
		if(!result){
			msgTL.set("参数校验失败：[单值："+val+"]->规则["+rule+"]");
		}
		return result;
	}
	...
	public boolean check(Object obj, String config){
		...
		while(keysIt.hasNext()){//一次是一个属性的校验
			...
			if(attObj == null){
				if(option.startsWith("required")){
					msgTL.set("参数校验失败：["+key+":"+val+"]->规则[required]");
					return false;
				}else{
					return true;//可选的项目，但是没有值，直接返回true
				}
			...
			if(option.startsWith("required")){
				...
				if(option.contains(";")){//多则校验
					...
					for (int i = 0; i < rules.length; i++) {//规则循环校验
						...
						if(!result){
							msgTL.set("参数校验失败：["+key+":"+val+"]->规则["+rules[i]+"]");
							return result;//一旦校验到false就返回
						}
					}
				}else{
					result = singleValid(option, val, jo, result);
					if(!result){
						msgTL.set("参数校验失败：["+key+":"+val+"]->规则["+option+"]");
						return result;//一旦校验到false就返回
					}
				}
	...
	private boolean optionChecker(String val, String option) {
		...
		if("email".equals(option)){
			result = patternEmail(val);
		}
		if("mobile".equals(option)){
			result = mobile(val);
		}
		if("integer".equals(option)){
			result = patternInt(val);
		}
		if("date".equals(option)){
			result = patternDate(val);
		}
		if("time".equals(option)){
			result = patternHHmmss(val);
		}
		if("datetime".equals(option)){
			result = patternDatetime(val);
		}
		if("timestamp".equals(option)){
			result = patternTimestamp(val);
		}
	...
	private boolean optionArgsChecker(String val, String option, String[] args) {
		...
		if("length".equals(option)){
			result=lengthCheck(val, args[0]);
		}
		if("pattern".equals(option)){
			result = pattern(val, args[0]);
		}
		if("integer".equals(option)){
			result = integer(val, args[0]);
		}
		if("range".equals(option)){
			result = range(val, args[0]);
		}
		if("compare".equals(option)){
			result = compare(val, args);
		}
	...
	private boolean compare(String val1, String[] args) {
		...
		if(val1.contains(":") || val1.contains("-")){
			if(">=".equals(args[0])){
				result = val1.compareTo(args[1])>=0;
			}
			if("<=".equals(args[0])){
				result = val1.compareTo(args[1])<=0;
			}
	...
	private boolean length(String val, long len1, long len2){
		...
		return len1<=val.length() && val.length()<=len2;
	}
