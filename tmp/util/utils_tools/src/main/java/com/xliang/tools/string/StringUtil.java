package com.xliang.tools.string;
...
public class StringUtil
{
     ...
     * @return
     */
    public static String randomPasswordByName(String name, int length)
    {
        if (isEmpty(name))
        {
            return "";
        ...
        Random rand = new Random();
        StringBuffer letters = new StringBuffer("0123456789");
        letters.append(name.toLowerCase()).append(name.toUpperCase());
        char[] base = letters.substring(0, letters.length()).toCharArray();
        StringBuffer result = new StringBuffer();
        for (int i = 0; i < length; i++)
        {
            result.append(base[rand.nextInt(letters.length())]);
        }
        return result.toString();
    }
    ...
    {
        System.out.println(randomPasswordByName("xiongliang", 6));
    }
}
