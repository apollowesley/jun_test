...
Page({
  ...
  onLoad: function (options) {
    ...
    // };
    //  wx.request({
    //    url: pathUrl + '/rest/fxjMqLogController' ,
    //   data: data,
    //   method: 'POST',
  ...
  syncLayout(media) {
    ...
    for (let i = 0; i < sizes.length; i++) {
      ...
      if(item.holding){
        ...
        if (i == 0) {  // 自己
            item.left = 0;
            item.top = (wx.getSystemInfoSync().windowHeight) - 214;
            item.width = 100;
            item.height = 150;
        } else if (i == 1) {  // 对方
            item.left = 0;
            item.top = 0;
            item.width = (wx.getSystemInfoSync().windowWidth);
            item.height = (wx.getSystemInfoSync().windowHeight) - 214;
        } else {
            item.left = (i-1) * 100 + 10;
            item.top = (wx.getSystemInfoSync().windowHeight) - 214;
            item.width = 100;
            item.height = 150;
        }
