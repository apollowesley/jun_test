package durcframework.test.student.controller;

import org.durcframework.core.GridResult;
import org.durcframework.core.SearchPojo;
import org.durcframework.core.controller.SearchController;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseBody;

import durcframework.test.student.entity.ExtStudentSearch;
import durcframework.test.student.entity.Student;
import durcframework.test.student.service.StudentService;

/**
 * 支持Extjs查询
 * @author hc.tang
 *
 */
@Controller
public class ExtjsPagingController extends SearchController<Student, StudentService> {
	
	// http://localhost/durcframeworkTest/listExtjsPagging.do?start=0&limit=10
	@RequestMapping("listExtjsPagging.do")
	public @ResponseBody GridResult listExtjsPagging(SearchPojo searchEntity) {
		return this.query(searchEntity);
	}
	
	// http://localhost/durcframeworkTest/listExtjsPagging2.do?start=3&limit=5&schName=jim
	@RequestMapping("listExtjsPagging2.do")
	public @ResponseBody GridResult listExtjsPagging2(ExtStudentSearch searchEntity) {
		return this.query(searchEntity);
	}
}
