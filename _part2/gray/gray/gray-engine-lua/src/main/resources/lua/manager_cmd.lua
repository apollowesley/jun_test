...
local function manager(applicationId,logIsOpen,cmd)
	...
	local curUrl = serverUrl .. "server/config?applicationId="..applicationId.."&engineType="..engineType
 
	local applicationConfigJson = httpPost(curUrl,"POST","")
	...
	if((cmd == "startApplicationGray" or cmd == "startServiceGray") and application.status ~= 0) then	
		...
		--获取当前upstream对应的配置
		local createGray = httpPost(upstreamUrlManager .. "upstream/" .. grayUpstream,"POST",application.config.gray_nginx)
        local createNew = httpPost(upstreamUrlManager .. "upstream/" .. newUpstream,"POST",application.config.deploy_nginx)	
...
function reloadInit(applicationIds)
	...
	local function timerTask()
		for k,v in ipairs(applicationIds) do
			--拉取本应用对应的操作命令列表
			local version = strategyCache:get(v .. "_version")
			if(not(version)) then
                    version = 0
            end
			local curUrl = serverUrl .. "server/cmd?applicationId="..v.."&version="..version
	        local cmdJson = httpPost(curUrl,"POST","")
	        if(cmdJson) then
		        local obj, pos, err = dkjson.decode(cmdJson, 1, nil)
				if(obj.success and obj.code =="J000000") then
					--解析json成功，进行进本数据校验和数据缓存
					local obj = obj.obj
					--如果返回的命令条数过多，直接拉取全部配置即可
					if(obj.total > 1) then 
						manager(v,0,"startApplicationGray")
						--设置版本号为最大版本号
						strategyCache:set(v .. "_version",obj.cmd.version)
					elseif(obj.cmd) then 
						--如果缓存中的version不存在，则启动项目，并且设置缓存版本为最大版本号
						manager(v,0,obj.cmd.grayCmd)
						strategyCache:set(v .. "_version",obj.cmd.version)
					end
				end
			end
