    	...
    	for(String id : ids){
	    	if (StringUtils.isNotBlank(id)) {
	    		...
	    		if("stopServiceGray".equals(cmd) && applicationServiceEntity.getStatus().intValue() ==0 ){
	    			...
	    			//数据完整性校验
	    			Result<Application> result = publishCompleteCheck(application.getApplicationId());
	    		...
	    		if("stopServiceGray".equals(cmd)){
		    		...
		    		if(application.getStatus().intValue() ==0 ){
	    		...
	    		//更新命令到命令表
		    	application.setUpdateUser(getStaffName());
		    	applicationCmdService.saveCmd(cmd,application);
	        }
    	}
