...
public class MybatisAutoConfiguration {
	...
	private List<FillHandler<?>> buildFills(Map<String,String> fillMap) {
		Set<Entry<String, String>> entrySet = fillMap.entrySet();
		List<FillHandler<?>> fillList = new ArrayList<>(entrySet.size());
		String className = "",parameter = "";
		try {
			for (Entry<String, String> entry : entrySet) {
				className = entry.getKey();
				parameter = entry.getValue();
				FillHandler<?> handler = (FillHandler<?>)buildInstance(className, parameter);
			...
			String errorMsg = "属性[mybatis.fill." + className + "=" + parameter + "]设置错误,检查类是否存在并继承net.oschina.durcframework.easymybatis.handler.FillHandler";
