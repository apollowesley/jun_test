...
import tom.kit.json.annotations.JSON;
 ...
 */
class JSONWriter {
	private static final Logger log = LoggerFactory.getLog(JSONWriter.class);
	...
	private void map(Map map, Method method) throws JSONException {
		...
		while (it.hasNext()) {
			...
			if (!warnedNonString && !(key instanceof String)) {
				log.error("JavaScript doesn't support non-String keys, using toString() on " + key.getClass().getName());
	...
	private void date(Date date, Method method) {
		...
		if (date instanceof Timestamp) {
			this.formatter = new SimpleDateFormat(JSONUtil.TIME_FORMAT);
		} else {
			this.formatter = new SimpleDateFormat(JSONUtil.DATE_FORMAT);
		}
