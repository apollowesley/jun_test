...
(function() {
	...
	Draggable = (function() {
		function Draggable($container, selector, settings, callbacks) {
		...
		Draggable.prototype.bind = function(method) {
			...
			return $(document)[method]('mouseup touchend touchcancel',
					this.ended);
		};
		...
		Draggable.prototype.began = function(event) {
			...
			return (_ref = this.callbacks) != null ? typeof _ref.began === "function" ? _ref
					.began(event)
					: void 0
					: void 0;
		};
		...
		Draggable.prototype.ended = function(event) {
			...
			delete this.$target;
			delete this.origin;
			return (_ref = this.callbacks) != null ? typeof _ref.ended === "function" ? _ref
					.ended(event)
					: void 0
					: void 0;
		};
		...
		Draggable.prototype.moved = function(event) {
			...
			event.stopPropagation();
			this.$target.css({
				left : this.coordinate(event).pageX
						+ $(".pageContent", navTab.getCurrentPanel())
								.scrollLeft() - this.origin.x,
				top : this.coordinate(event).pageY
						+ $(".pageContent", navTab.getCurrentPanel())
								.scrollTop() - this.origin.y
			});
			this.dragged = this.$target;
			return (_ref = this.callbacks) != null ? typeof _ref.moved === "function" ? _ref
					.moved(event)
					: void 0
					: void 0;
		};
	...
	Gridly = (function() {
		...
		Gridly.prototype.ordinalize = function($elements) {
			...
			_results = [];
			for (i = _i = 0, _ref = $elements.length; 0 <= _ref ? _i <= _ref
		...
		Gridly.prototype.draggable = function(method) {
			if (this._draggable == null) {
				this._draggable = new Draggable(this.$el, '> *', this.settings,
						{
		...
		Gridly.prototype.draggingBegan = function(event) {
			...
			return (_ref = this.settings) != null ? (_ref1 = _ref.callbacks) != null ? typeof _ref1.reordering === "function" ? _ref1
					.reordering($elements)
					: void 0
					: void 0
					: void 0;
		...
		Gridly.prototype.draggingEnded = function(event) {
			 ...
			 * _ref.optimize : void 0) || this.optimize)(this.$sorted());
			 */
			$elements = this.$sorted();
			this.ordinalize($elements);

			setTimeout(this.layout, 0);
			return (_ref = this.settings) != null ? (_ref1 = _ref.callbacks) != null ? typeof _ref1.reordered === "function" ? _ref1
					.reordered($elements, event)
					: void 0
					: void 0
					: void 0;
		};
		...
		Gridly.prototype.draggingMoved = function(event) {
			if (this.settings.draggable !== false) {
				 ...
				 * _ref1.optimize : void 0) || this.optimize)($elements);
				 */

				for (i = _j = 0, _ref2 = $elements.length; 0 <= _ref2 ? _j < _ref2
						: _j > _ref2; i = 0 <= _ref2 ? ++_j : --_j) {
		...
		Gridly.prototype.size = function($element) {
			return (($element.data('width') || $element.width()) + this.settings.gutter)
		...
		Gridly.prototype.position = function($element, columns) {
			...
			height = Infinity;
			column = 0;
			for (i = _i = 0, _ref = columns.length - size; 0 <= _ref ? _i < _ref
					: _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
			...
			for (i = _j = column, _ref1 = column + size; column <= _ref1 ? _j < _ref1
					: _j > _ref1; i = column <= _ref1 ? ++_j : --_j) {
				columns[i] = height
		...
		Gridly.prototype.structure = function($elements) {
			...
			columns = (function() {
				...
				_results = [];
				for (i = _i = 0, _ref = this.settings.columns; 0 <= _ref ? _i <= _ref
						: _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
					_results.push(0);
				}
				return _results;
			}).call(this);
			for (index = _i = 0, _ref = $elements.length; 0 <= _ref ? _i < _ref
					: _i > _ref; index = 0 <= _ref ? ++_i : --_i) {
		...
		Gridly.prototype.layout = function() {
			...
			for (index = _i = 0, _ref1 = $elements.length; 0 <= _ref1 ? _i < _ref1
					: _i > _ref1; index = 0 <= _ref1 ? ++_i : --_i) {
				$element = $($elements[index]);
		...
		Gridly.prototype.optimize = function(originals) {
			...
			while (originals.length > 0) {
				...
				index = 0;
				for (index = _i = 0, _ref = originals.length; 0 <= _ref ? _i < _ref
						: _i > _ref; index = 0 <= _ref ? ++_i : --_i) {
	...
	$.fn.extend({
		gridly : function() {
			...
			option = arguments[0], parameters = 2 <= arguments.length ? __slice
					.call(arguments, 1) : [];
			if (option == null) {
				option = {};
			}
