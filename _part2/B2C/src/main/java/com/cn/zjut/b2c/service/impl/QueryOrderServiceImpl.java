...
import com.cn.zjut.b2c.sdk.SDKConfig;
...
public class QueryOrderServiceImpl implements QueryOrderServiceInter {
	...
	public Map<String, String> getRequestData(String orderId, String txnTime) {
		...
		return AcpService.sign(data,DemoBase.encoding_UTF8);//报文中certId,signature的值是在signData方法中获取并自动赋值的，只要证书配置正确即可。
	}
			...
			if(AcpService.validate(rspData, DemoBase.encoding_UTF8)){
				...
				if("00".equals(rspData.get("respCode"))){//如果查询交易成功
					...
					if("00".equals(origRespCode)){
						//交易成功，更新商户订单状态
						//TODO
						LogUtil.writeErrorLog("交易成功，更新商户订单状态 ");
					}else if("03".equals(origRespCode) ||
							 "04".equals(origRespCode) ||
							 "05".equals(origRespCode)){
						//需再次发起交易状态查询交易 
						//TODO
						LogUtil.writeErrorLog("需再次发起交易状态查询交易 ");
					}else{
						//其他应答码为失败请排查原因
						//TODO
						LogUtil.writeErrorLog("其他应答码为失败请排查原因");
					}
				}else{//查询交易本身失败，或者未查到原交易，检查查询交易报文要素
					//TODO
					LogUtil.writeErrorLog("查询交易本身失败，或者未查到原交易，检查查询交易报文要素");
				}
			}else{
