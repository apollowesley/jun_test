 ...
 * 日期:2013-10-24
 */
public class WordFirstCharLower implements TemplateMethodModel {
	...
	public Object exec(List args) throws TemplateModelException {
		...
		for(int i=0;i<wordArray.length;i++){
			if(i==wordArray.length-1){
				if(!endWithSplit){
					buff.append(StringHelper.toFirstLower(wordArray[i]));
				}else{
					buff.append(StringHelper.toFirstLower(wordArray[i])).append(split);
				}
			}else{
				buff.append(StringHelper.toFirstLower(wordArray[i])).append(split);
			}
