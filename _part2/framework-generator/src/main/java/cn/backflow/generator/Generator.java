...
import cn.backflow.generator.util.IOHelper;
import cn.backflow.generator.util.AntPathMatcher;
import cn.backflow.generator.util.FreemarkerHelper;
import cn.backflow.generator.util.GeneratorException;
import cn.backflow.generator.util.FileHelper;
...
public class Generator {
	...
	void setTemplateRootDir(File templateRootDir) {
	...
	void deleteOutRootDir() throws IOException {
	 ...
	 * @throws Exception
	 */
	Generator generateBy(Map templateModel, Map filePathModel) throws Exception {
	 ...
	 * @throws Exception
	 */
	Generator deleteBy(Map templateModel, Map filePathModel) throws Exception {
	...
	private void processTemplateRootDirs(Map templateModel, Map filePathModel, boolean isDelete) throws Exception {
		...
		for (Object templateRootDir1 : this.templateRootDirs) {
			File templateRootDir = (File) templateRootDir1;
	...
	private List<Exception> scanTemplatesAndProcess(File templateRootDir, Map templateModel, Map filePathModel, boolean isDelete) throws Exception {
		...
		for (Object srcFile1 : srcFiles) {
			File srcFile = (File) srcFile1;
			try {
				if (isDelete) {
	...
	private class TemplateProcessor {
		private final GeneratorControl gg = new GeneratorControl();
	...
	static class GeneratorHelper {
		static boolean isIgnoreTemplateProcess(File srcFile, String templateFile, String includes, String excludes) {
		...
		static Configuration newFreeMarkerConfiguration(List<File> templateRootDirs, String defaultEncoding, String templateName)
				throws IOException {
			...
			conf.setAutoIncludes(availableAutoInclude);
			GLogger.trace("set Freemarker.autoIncludes:" + availableAutoInclude + " for templateName:" + templateName + " autoIncludes:"
					+ autoIncludes);
			return conf;
		}

		static List<String> getParentPaths(String templateName, String suffix) {
			...
			for (String anArray : array) {
				path.append(File.separator).append(anArray);
	...
	static class GeneratorModel implements java.io.Serializable {
		Map filePathModel;
		Map templateModel;

		GeneratorModel(Map templateModel, Map filePathModel) {
