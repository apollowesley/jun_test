...
<c:set var="isEdit" value="<@jspEl 'not empty '+classNameFirstLower+'.'+table.idColumn.columnNameFirstLower/>" />
<div class="container">
  <section class="page-title">
    <div class="row">
      <div class="col-sm-8">
        <h1 class="main-title"><@jspEl 'isEdit ? \'编辑\' : \'新增\' '/>权限${table.tableAlias}</h1>
  ...
  <div class="row row-fullw bg-white">
    <form:form modelAttribute="${classNameFirstLower}" action="<@jspEl 'ctx'/>${classNameLower}/<@jspEl 'isEdit ? '+classNameFirstLower+'.'+table.idColumn.columnNameFirstLower +'.toString().concat(\'/update\') : \'create\'' />" method="post" class="validation-form" role="form">
      ...
      <div class="col-lg-6 col-md-12">
        <div class="panel panel-white">
          ...
          <#elseif !column.isNullable()>
            <div class="form-group">
              <label>${column.columnAlias }<i class="symbol required" aria-required="true"></i><form:input path="${column.columnNameLower}" placeholder="${column.columnAlias }" class="form-control required" aria-required="true" /></label>
            </div>
          </#if>
