package org.coody.czone.common.cache;
...
public class RedisCache implements CoodyCacheFace{
	
	private BaseLogger logger=BaseLogger.getLogger(RedisCache.class);
	...
	public Map<String, Object> getCacheBatch(String... keys) {
		...
		try {
			...
			for(String key:responses.keySet()){
				...
				Object value = unserialize(response.get());
				map.put(key, value);
			}
			return map; 
		} catch (Exception e) {
	...
	public void lBeanPushHead(String key, Object... values) {
		...
		try {
			jedis = jedisPool.getResource();
			for (Object value : values) {
				jedis.lpush(key.getBytes("ISO-8859-1"), serialize(value));
	...
	public Long lrem(String key, int count, Object value) {
		...
		try {
			jedis = jedisPool.getResource();
			Long ret = jedis.lrem(key.getBytes("ISO-8859-1"), count, serialize(value));
			if (ret != null) {
				return ret;
			}
			return 0L;
		...
		try {
			if (map != null && map.size() > 0) {
				...
				for (String field : map.keySet()) {
					...
					byte[] v = serialize(map.get(field));
					hash.put(k, v);
				}
				jedis = jedisPool.getResource();
	...
	public List<?> hmget(String key, Object... fieldName) {
		...
		try {
			...
			if (!status) {
				logger.error("###hmget 反序列化存在失败对象，key=" + key);
			}
			return ret;
		} catch (Exception e) {
			delCache(key);
